...
| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `key` | string | yes | Config path (e.g., `"llm.api_key"`, `"llm.provider"`, `"embedding.model"`) |
...
Built-in providers: OpenAI, DeepSeek, Moonshot, Zhipu, SiliconFlow, Volcengine, Custom.
...
Semantic search over `.claude/memory/*.md`, `.claude/rules/*.md`, and project root `MEMORY.md`. Preferred over `search_memory` for conceptual queries.
...
Read-only access to configuration files via `claude://` URIs.
...
Filenames must end in `.md` and cannot contain `..` or path separators.
